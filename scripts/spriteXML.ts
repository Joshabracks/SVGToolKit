...
class XMLSprite {
    ...
    inbetweenFrame = (frame) => {
        ...
        let colorVal;
        function colorValues(string) {
            let val1: number
            let val2: number
            let val3: number
            if (string[0] === '#') {
                if (string.length == 7) {
                    val1 = parseInt(string[1] + string[2], 16)
                    val2 = parseInt(string[3] + string[4], 16)
                    val3 = parseInt(string[5] + string[6], 16)
                }
                if (string.length == 4) {
                    val1 = parseInt(string[1], 16)
                    val2 = parseInt(string[2], 16)
                    val3 = parseInt(string[3], 16)
                }
            } else if (string.slice(0, 2) == 'rgb') {
        ...
        for (let i = 0; i < frame.attributes; i++) {
            if (topAttributes[i].value != botAttributes[i].value) {
                let attr = frame.attributes[i]
                if (attr.name === 'fill' || attr.name === 'stroke') {
                    ...
                    color += ')'
                    frame.setAttribute(attr.name, color)
